...
# Generated by `handoff init --planner cursor` and by the Cursor Executor's
# per-run delivery. Only these exact locations; other .cursor content is code.
...
def is_workflow_path(rel: str) -> bool:
    ...
    if normalized == LOCAL_SETTINGS:
        return True
    if normalized == PLANNER_SKILL_DIR or normalized.startswith(f"{PLANNER_SKILL_DIR}/"):
        return True
